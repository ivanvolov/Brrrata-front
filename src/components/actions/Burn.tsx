...
export default function Burn() {
  ...
  const { data: brrrBalance, queryKey: queryKeyBRRRB } = useReadContract({
    abi: brrrataABI,
  ...
  useEffect(() => {
    ...
    queryClient.invalidateQueries({ queryKey: queryKeyBRRRB });
        ...
        <div className="flex flex-col space-y-2">
          <div className="flex items-center justify-between">
            ...
            <span className="text-gray-700" id="rangeValue">
              Balance: {!brrrBalance ? '...' : format(toBN(brrrBalance))} BRRR
            </span>
          </div>
          <input
            type="range"
